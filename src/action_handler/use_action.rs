...
fn execute_init_commands(commands: Vec<String>) {
    for command in commands {
        let mut command_split = command.split(" ");
        let command_name = command_split.next().unwrap_or_default();
        let command_args = command_split.collect::<Vec<&str>>();
        let mut command = std::process::Command::new(command_name);
        command.args(command_args);
        command
            .spawn()
            .expect("Error encountered while executing the command");
    }
}
...
pub fn handle_action(kettle_name: &str, destination_folder: &str, kettle_repo_path: &str) {
    if Path::new(destination_folder).is_dir() && destination_folder != "." {
        ...
        execute_init_commands(recipe_json.init);
        if destination_folder != "." {
