...
fn init_pre(action_args: &mut std::env::Args, kettle_repo_path: &str, kettle_name_warning: &str) {
...
fn use_pre(action_args: &mut std::env::Args, kettle_name_warning: &str, kettle_repo_path: &str) {
    let kettle_name = action_args.nth(0).unwrap_or_default();

    if check_default(&kettle_name, kettle_name_warning) {
        let dest_folder = action_args.nth(0).unwrap_or_default();
        if check_default(&dest_folder, "⚠️  No destination folder was given") {
            use_action::handle_action(&kettle_name, &dest_folder, kettle_repo_path);
        }
    }
}

fn get_pre(action_args: &mut std::env::Args, kettle_repo_path: &str, kettle_name_warning: &str) {
    let kettle;
